...
const DisplayUpvotedProfileQuotesForm: FC = () => {
  ...
  const { data, isLoading, fetchNextPage, hasNextPage, isFetchingNextPage } =
    useInfiniteQuery(
      "fetchProfileUpvotedQuotes",
      ({ pageParam = 1 }) => API.fetchProfileUpvotedQuotes(userID, pageParam),
      {
        getNextPageParam: (lastPage) => {
          const { meta } = lastPage.data;
          const nextPage = parseInt(meta.page, 3) + 1;
...
export default DisplayUpvotedProfileQuotesForm;
